                    ...
                    rows.append(
                        {
                            ...
                            "stratum": f"hard:{hc}",
                            "table": "-",
                            "landscapes": int(round(n_syn * frac * hfrac)),
                        }
                    )
            else:
                rows.append(
                    {
                        ...
                        "stratum": gen,
                        "table": "-",
                        "landscapes": int(round(n_syn * frac)),
                    }
                )
            ...
            rows.append(
                {
                    ...
                    "solves": int(round(n * spec["per_landscape"])),
                }
            )
    ...
    rows.append(
        {
            "tier": abl["tier"],
            "family": "synthetic",
            "config": "points (4-neighbour ablation)",
            "tasks": "T1,T2",
            "solves": int(round(n_syn_s * abl["fraction_of_synthetic"])),
        }
    )
    return pd.DataFrame(rows)
...
def cost_table(
    ...
    land: pd.DataFrame,
    conf: pd.DataFrame,
    omni_scale: dict | None = None,
    omni_option: str | None = None,
) -> pd.DataFrame:
    ...
    for t in TIERS:
        ...
        for r in conf[conf.tier == t].itertuples():
            key = (
                "points"
                if r.config.startswith("points")
                else ("wall_to_wall" if r.config.startswith("wall") else r.config)
            )
        ...
        sec *= oh
        gb = (
            n_syn * cfg["storage_mb_per_landscape"]["synthetic"][t]
            + n_real * cfg["storage_mb_per_landscape"]["real"][t]
        ) / 1000
        per_land = sec / max(n_syn + n_real, 1)
        rows.append(
            {
                "tier": t,
                "landscapes": n_syn + n_real,
                "cpu_s_per_landscape": round(per_land, 1),
                "peak_rss_gb": round(cfg["peak_rss_mb"][t] / 1000, 1),
                "cpu_hours": round(sec / 3600, 0),
    ...
    tot = {
        "tier": "total",
        "landscapes": int(df.landscapes.sum()),
        "cpu_s_per_landscape": None,
        "peak_rss_gb": None,
        "cpu_hours": float(df.cpu_hours.sum()),
        "storage_gb": round(float(df.storage_gb.sum()), 1),
    }
...
def wallclock(cpu_hours: float) -> pd.DataFrame:
    rows = [
        {"concurrent_cores": c, "wall_days": round(cpu_hours / c / 24, 1)} for c in (100, 500, 1000)
    ]
    return pd.DataFrame(rows)
...
def main() -> None:
    ...
    ap.add_argument(
        "--omniscape-scale", default="", help="e.g. XL=0.265,XXL=0.258 to price a coarser block"
    )
    ap.add_argument("--json", default=None)
    ap.add_argument(
        "--omniscape-option",
        default=None,
        help="phase5 | fidelity (default: omniscape_choice in the config)",
    )
    args = ap.parse_args()
    ...
    print(
        f"## Ladder: {args.ladder} {ladder}; Omniscape option: {opt} {cfg['omniscape_options'][opt]}\n"
    )
    print("### Landscapes per tier × family × stratum/table\n")
    piv = land.pivot_table(
        index=["family", "stratum", "table"],
        columns="tier",
        values="landscapes",
        aggfunc="sum",
        fill_value=0,
    ...
    piv2 = conf.pivot_table(
        index=["family", "config", "tasks"],
        columns="tier",
        values="solves",
        aggfunc="sum",
        fill_value=0,
    ...
    if args.json:
        json.dump(
            {
                "ladder": ladder,
                "landscapes": land.to_dict("records"),
                "solves": conf.to_dict("records"),
