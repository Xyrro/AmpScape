...
LADDER = {"S": 100_000, "M": 50_000, "L": 20_000, "XL": 4_000}     # recommended v1.0 landscapes per tier
TRAIN_SHARE = {"S": 0.61, "M": 0.72, "L": 0.61, "XL": 0.25 * 0.61}   # dev-measured train share (S, M); XL: 25 % train/val slice
...
def main():
    ...
    for d in sorted(pathlib.Path(a.runs).glob("*_T*")):
        ...
        rows.append({"model": cfg["model"], "task": cfg["task"], "params_M": cfg["n_params"] / 1e6, "n_train": n_train, "epochs": len(hist),
                     "best_epoch": best["epoch"], "epoch_s": ep_s, "gpu_h_per_epoch": ep_s / 3600, "train_gpu_h": train_h,
                     "s_per_sample_epoch": ep_s / n_train, "best_val": best["val_loss"]})
    lines = ["# GPU budget (Phase 10 dev runs, L40S, bf16 autocast except FNO)", "",
             "| model | task | params (M) | train items | epochs run | best epoch | s / epoch | GPU-h / epoch | training GPU-h |",
             "|---|---|---|---|---|---|---|---|---|"]
    for r in rows:
        lines.append(f"| {r['model']} | {r['task']} | {r['params_M']:.2f} | {r['n_train']} | {r['epochs']} | {r['best_epoch']} | {r['epoch_s']:.1f} | "
                     f"{r['gpu_h_per_epoch']:.4f} | {r['train_gpu_h']:.3f} |")
    total_dev = sum(r["train_gpu_h"] for r in rows)
    lines += ["", f"Dev training total: **{total_dev:.2f} GPU-h** ({len(rows)} runs); evaluation passes add a few minutes each.", "",
    ...
    for r in rows:
        ...
        for tier, n in LADDER.items():
            ...
            e = max(e, 5)
            eps[tier] = e
            per[tier] = n_train * e * r["s_per_sample_epoch"] * PIXELS[tier] / PIXELS["S"] / 3600 * SEEDS
        tot = sum(per.values())
        grand += tot
