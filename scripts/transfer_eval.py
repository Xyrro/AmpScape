...
def main() -> None:
    ...
    ap.add_argument(
        "--keep-predictions",
        default="test_id",
        help="which splits keep predictions.h5 after the metrics ('all', 'none' or a comma list); XL predictions "
    ...
    graph = name == "gnn"
    # the scale-aware variant's inverse uses the EVALUATION tier's factor (radius / pixel count) — that is the point
    print(
        f"target normalisation: {cfg.get('target_norm', 'abs')} (trained at {cfg['tier']}, evaluated at {a.tier})",
        flush=True,
    )
    ...
    for split in [s.strip() for s in a.splits.split(",") if s.strip()]:
        ...
        if not (pdir / "predictions.h5").exists() or not (pdir / "meta.json").exists():
            dl = torch.utils.data.DataLoader(
                Items(
                    ds, task, stats, graph, extra, multiscale, a.tier, cfg.get("target_norm", "abs")
                ),
                batch_size=1,
                shuffle=False,
        ...
        if "all" not in keep and split not in keep:
            ...
            if h5.exists():
                ...
                h5.unlink()
                (pdir / "predictions_deleted.json").write_text(
                    json.dumps(
                        {
                            "deleted_at": summary["updated_at"],
                            "bytes": size,
                            "reason": "scratch quota",
                        }
                    )
                )
        print(
