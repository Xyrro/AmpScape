...
KEYS = ["mae_log10eps", "rel_l2", "top5_iou", "pinch_recall", "spearman", "corridor_dice_q10", "ssim"]
GROUPS = [("S_S_test_id", "test_id"), ("S_S_test_ood", "test_ood"), ("S_S_ood_region", "ood_region"),
          ("published_S_test_ood_published", "published S"), ("published_XXL_test_ood_published", "published XXL")]
...
def main():
    ...
    for d in sorted(pathlib.Path(a.runs).glob("*_T*")):
        ...
        for tag, name in GROUPS:
            ...
            rows.append({"model": d.name, "task": t, "split": name, "n": e.get("n_rows"), **{k: m.get(k) for k in KEYS}, "speedup": sp,
                         "params_M": cfg["n_params"] / 1e6, "epochs": len(r["history"]), "gpu_h": (r["history"][-1]["cum_gpu_h"] if r["history"] else None)})
    ...
    for split in ("test_id", "test_ood", "ood_region"):
        ...
        for t, agg in r["per_task"].items():
            if t not in ("T1", "T3", "T4"):
                continue
            rows.append({"model": "coarsen4 (non-learned)", "task": t, "split": split, "n": agg.get("mae_log10eps", {}).get("n"),
                         **{k: agg.get(k, {}).get("mean") for k in KEYS}, "speedup": (agg.get("speedup") or {}).get("speedup_median"),
                         "params_M": None, "epochs": None, "gpu_h": None})
    lines = ["# Learned baselines on the dev subset (tier S, single seed) vs the coarsen-×4 baseline", "",
    ...
    for r in sorted(rows, key=lambda r: (r["task"], order[r["split"]], r["model"])):
        lines.append(f"| {r['task']} | {r['split']} | {r['model']} | {r['n']} | " + " | ".join(fmt(r[k]) for k in KEYS) +
