...
def block_study() -> tuple[str, str]:
        ...
        rows.append(
            {
                "tier": d["tier"],
                "radius": d["radius"],
                "blocks": f"{d['blocks'][0]} vs {d['blocks'][1]}",
                "b/r": f"{d['blocks'][0] / d['radius']:.3f} vs {d['blocks'][1] / d['radius']:.3f}",
                "sample": d["sample"][:8],
                "rel_L2_cum": c["rel_l2"],
                "max_diff/max_cum": c["max_abs_diff_over_max"],
                "pearson_cum": c["pearson"],
                "rel_L2_normalized": d["normalized"]["rel_l2"],
                "t_fine_s": round(d["t_a"]),
                "t_coarse_s": round(d["t_b"]),
            }
        )
    ...
    for (tier, blocks), g in sorted(df.groupby(["tier", "blocks"]), key=lambda kv: order[kv[0][0]]):
        ...
        if b < a:  # anchor: alternative is the finer (exact when b == 1) setting
            verdict.append(
                f"- **{tier} anchor, block {a} vs {b} (radius {r}, b/r {a / r:.2f} vs {b / r:.2f})**: the standard block {a} "
            ...
            ok = mx < 0.01
            verdict.append(
                f"- **{tier} coarsening, block {a} vs {b} (radius {r}, b/r {a / r:.2f} vs {b / r:.2f})**: relative L2 "
...
def main() -> None:
    ...
    hc = {
        k: sum(x["landscapes"] for x in nl if x["stratum"] == f"hard:{k}")
        for k in (
            "high_contrast_1e5",
            "rmax_saturated",
            "narrow_corridor",
    ...
    out = TEMPLATE.read_text().format(
        land_rec=sect(rec, "### Landscapes per tier"),
        solves_rec=sect(rec, "### Solves per tier"),
        land_brief=sect(brief, "### Landscapes per tier"),
        tot_land_r=sum(x["landscapes"] for x in nl),
        tot_solves_r=sum(x["solves"] for x in jr["solves"]),
        tot_land_b=sum(x["landscapes"] for x in nb),
        tot_solves_b=sum(x["solves"] for x in jb["solves"]),
        cost_rec=sect(rec, "### Cost"),
        wall_rec=sect(rec, "### Wall-clock"),
        cost_brief=sect(brief, "### Cost"),
        wall_brief=sect(brief, "### Wall-clock"),
        cost_rec_p5=sect(rec_p5, "### Cost"),
        wall_rec_p5=sect(rec_p5, "### Wall-clock"),
        block_study=study,
