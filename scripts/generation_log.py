...
def tier_summary(build: pathlib.Path) -> dict:
    ...
    solved = len(
        {p.stem.replace(".outputs", "") for p in (build / "outputs").glob("shard-*.outputs.h5")}
        | up_names
        | {p.stem for p in (build / "shards").glob("shard-*.h5")}
        | {p.stem for p in (build / "shards").glob("shard-*.ok")}
    )
    ...
    idx = (
        pd.concat([pd.read_parquet(p) for p in rows], ignore_index=True) if rows else pd.DataFrame()
    )
    ...
    for m in up:
        ...
        gb_hub += (
            sum(p.get("bytes", 0) for p in d.get("parts", {}).values()) / 1e9
            if "parts" in d
            else 0.0
        )
    try:
        ...
        gb_hub = hub_gb(
            f"{__import__('os').environ.get('HF_ORG', 'Xirro')}/AmpScape",
            prefix=f"data/{man.tier.iloc[0]}/",
        )
...
def core_hours(since: str) -> float:
    r = subprocess.run(
        ["sacct", "-S", since, "-u", "yxiao413", "-o", "JobName,ElapsedRaw,AllocCPUS", "-P", "-n"],
        capture_output=True,
        text=True,
    ).stdout
    tot = 0.0
...
def main():
    ...
    rows = [
        tier_summary(pathlib.Path(b))
        for b in a.builds
        if (pathlib.Path(b) / "manifest.parquet").exists()
    ]
    ch = core_hours(a.since)
    stop = [
        f"{r['tier']}: QC fail {r['qc_fail_rate']:.2%}" for r in rows if r["qc_fail_rate"] > 0.01
    ] + [
        f"{r['tier']}: {r['upload_failed']} shard(s) failed to upload twice"
        for r in rows
        if r["upload_failed"]
    ]
    lines = [
    ...
    for r in rows:
        lines.append(
            f"| {r['tier']} | {r['shards']} | {r['solved']} | {r['finalized']} | {r['uploaded']} | {r['upload_failed']} | {r['invalid']} | "
            f"{r['qc_fail_rate']:.3%} | {r['gb_hub']:.1f} | {r['gb_local']:.1f} |"
        )
    lines += [
        "",
        f"Core-hours used since {a.since} (ampscape-* jobs): **{ch:.0f}**. Stop rule: "
        + ("**TRIGGERED — pause and report:** " + "; ".join(stop) if stop else "not triggered."),
        "",
    ]
    p = pathlib.Path(a.log)
    p.write_text(
        (p.read_text() if p.exists() else "# v1.0 generation log\n\n") + "\n".join(lines) + "\n"
    )
    print("\n".join(lines))
