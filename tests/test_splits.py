...
def test_block_ids_and_footprints():
    ...
    assert (
        len(ew.footprint_blocks(0.0, 0.0, 1_024_000)) == 2
    )  # XXL on a longitude boundary touches two cells
    assert ew.interior_bounds(ew.block_id(15, 10), 256_000) is not None  # XL fits inside a 20° cell
    assert (
        ew.interior_bounds(ew.block_id(15, 10), 1_024_000) is None
    )  # XXL does not (parent regions instead)
...
def test_no_cross_tier_overlap():
    ...
    touching = kept[
        kept.footprint_blocks.apply(lambda fb: any(b in ood for b in fb))
        & kept.region.str.startswith("cell:")
    ]
...
def test_synthetic_split_and_holdouts():
    ...
    cfg = {
        "ood": {
            "test_ood_scale": {"tiers": ["XL", "XXL"]},
            "test_ood_table": {"table": "forest_bird"},
            "test_ood_contrast": {"contrast": 10000},
        }
    }
    f = ood_flags(
        {"family": "real", "tier": "S", "split": "train", "resistance_table_id": "forest_bird"}, cfg
    )
    ...
    assert (
        f["test_ood_scale"] and f["test_ood_contrast"] and apply_holdouts("test_id", f) == "test_id"
    )
    f = ood_flags(
        {"family": "real", "tier": "M", "split": "test_id", "resistance_table_id": "amphibian"}, cfg
    )
