...
def test_pixel_metrics_hand_cases():
    ...
    assert (
        pixel.psnr(t, t) == float("inf")
        and pixel.ssim(np.random.rand(16, 16), np.random.rand(16, 16)) < 1.0
    )
...
def test_domain_metrics_hand_cases():
    ...
    assert (
        domain.pinch_points(t, m, 5.0, min_distance=1).sum() == 3
    )  # with min_distance=3 the two strong peaks suppress (5,5)
    p3 = np.zeros((10, 10))
    p3[2, 3], p3[7, 6], p3[0, 9] = 5, 5, 5
    assert domain.pinch_point_recall(
        p3, t, m, top_percent=5.0, min_distance=1, radius=1
    ) == pytest.approx(2 / 3)
    # Spearman: monotone transform -> 1, reversed -> -1
    a = np.arange(16, dtype=float).reshape(4, 4)
    assert domain.spearman(a**2, a) == pytest.approx(1.0) and domain.spearman(
        -a, a
    ) == pytest.approx(-1.0)
...
def test_reff_metrics_hand_cases():
    ...
    assert reff.nearest_neighbour_agreement(p2, t) == pytest.approx(
        1 / 3
    )  # node0->2, node1->2 (was 0), node2->0 (same)
...
def test_physics_metrics_hand_cases():
    ...
    c[0, 0], c[3, 3], c[0, 3] = (
        ...
        2.0,
        1.0,
    )  # cumulative over 3 pairs: labels 1,2 in 2 pairs each -> expected 2; label 3 in 2 pairs -> expected 2
    pairs = [(1, 2), (1, 3), (2, 3)]
