...
def test_points_contract(seed):
    ...
    assert s.focal_mask.dtype == np.int32 and sorted(np.unique(s.focal_mask).tolist()) == list(
        range(k + 1)
    )
...
def test_regions_from_habitat_patches():
    ...
    assert (
        sample_regions(R, nd, np.full((128, 128), 30, np.int16), CFG, np.random.default_rng(0))
        is None
    )
...
def test_advanced_contract(seed):
    ...
    assert (
        gnd.sum() > 0 and not (src[gnd > 0] > 0).any() and not src[nd].any() and not gnd[nd].any()
    )
...
def test_kirchhoff_residual_exact_solve():
    ...
    assert (
        kirchhoff_residual(R, nd, vmap, gnd.astype(float), grounded=src) > 1.0
    )  # swapped roles are caught
...
def test_kirchhoff_residual_supernode():
    ...
    assert (
        kirchhoff_residual(R, nd, vmap, inj, grounded=gnd) > 0.1
    )  # row-wise check would wrongly fail
