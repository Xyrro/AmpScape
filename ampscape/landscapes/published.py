...
class PublishedSource:
    ...
    crs_override: str | None = (
        None  # e.g. EPSG:32604 for the gallinule ASCII grids (no CRS in file)
    )
...
def tile_centres(
    src: PublishedSource, tier: str, max_tiles: int | None = None, seed: int = 0
...
def extract_published_tile(
    src: PublishedSource, lat: float, lon: float, tier: str, max_nodata_frac: float = 0.10
):
    ...
    win = (
        ds.window(xmin - 2 * native, ymin - 2 * native, xmax + 2 * native, ymax + 2 * native)
        .round_offsets()
        .round_lengths()
    )
    data = ds.read(
        1, window=win, boundless=True, fill_value=nodata if nodata is not None else np.nan
    ).astype(np.float64)
    ...
    prov = {
        ...
        "native_res_m": native,
        "tier": tier,
        "pixel_size_m": TIER_PIXEL[tier],
        "resampling": "geometric mean of R (area mean of log R)"
        if resamp == Resampling.average
        else "none (native resolution)",
        "rescale_factor": scale,
        "r_min": float(R[~nd].min()),
        "r_max": float(R[~nd].max()),
        "nodata_frac": float(nd.mean()),
        "epsg": grid.epsg,
