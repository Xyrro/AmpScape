...
def pinch_points(
    target: np.ndarray, mask: np.ndarray, top_percent: float = 5.0, min_distance: int = 3
) -> np.ndarray:
    """Local maxima (plateaus of zero flow excluded) that are also in the top-p % set."""
    return (
        local_maxima(target, mask, min_distance)
        & top_q_mask(target, mask, top_percent)
        & (target > 0)
    )


def pinch_point_recall(
    pred, target, mask=None, top_percent: float = 5.0, min_distance: int = 3, radius: int = 3
) -> float:
...
def all_domain(pred, target, mask=None) -> dict[str, float]:
    return {
        "top1_iou": top_q_iou(pred, target, mask, 1.0),
        "top5_iou": top_q_iou(pred, target, mask, 5.0),
        "top10_iou": top_q_iou(pred, target, mask, 10.0),
        "corridor_dice_q10": corridor_dice(pred, target, mask, 10.0),
        "pinch_recall": pinch_point_recall(pred, target, mask),
        "spearman": spearman(pred, target, mask),
    }
