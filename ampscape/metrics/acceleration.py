...
def run_warm_start_eval(source_h5: str | pathlib.Path, predictions_h5: str | pathlib.Path, out_json: str | pathlib.Path,
                        rtol: float = 1e-6) -> list[dict]:
    cmd = ["julia", f"--project={JULIA_PKG}", str(JULIA_PKG / "scripts" / "warm_start_eval.jl"), str(source_h5), str(predictions_h5),
           str(out_json), str(rtol)]
...
def summarize(records: list[dict]) -> dict[str, float]:
    ...
    return {"n_systems": len(records), "iters_zero_median": float(np.median(iz)), "iters_warm_median": float(np.median(iw)),
            "iter_reduction_median": float(np.median(1 - iw / np.maximum(iz, 1))),
            "time_zero_median_s": float(np.median(tz)), "time_warm_median_s": float(np.median(tw)),
