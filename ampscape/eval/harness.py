...
KIND_TASK = {"points": "T1", "wall_to_wall": "T1W", "regions": "T1R", "advanced": "T3", "omniscape": "T4"}
...
def evaluate_sample(kind: str, gs: h5py.Group, gc: h5py.Group, gp: h5py.Group, R: np.ndarray, nd: np.ndarray) -> dict:
    out: dict = {}
    o = gc["outputs"]
    ...
    if kind in ("points", "wall_to_wall", "regions"):
        if "cum_current" in gp:
            ...
            if "pairwise_current" in gp:      # per-pair maps: a unit source/ground pixel carries exactly 1 A
                pc = gp["pairwise_current"][...]
                pi = o["pair_index"][...]
                errs = [physics.focal_current_error(pc[q], np.where(np.isin(focal, pi[q]), focal, 0), None) for q in range(pc.shape[0])]
        ...
        if "voltage" in gp and "labels" in o:
            ...
            inj[src] = 1.0 / src.sum()
            out["phys_kirchhoff_residual"] = physics.kirchhoff_residual(R, nd, v0.astype(np.float64), inj, grounded=gnd,
        ...
        if "voltage" in gp:
            ...
            out["phys_kirchhoff_residual"] = physics.kirchhoff_residual(R, nd, gp["voltage"][...].astype(np.float64),
...
def evaluate(pred_dir: str | pathlib.Path, split: str | list[str], root: str | pathlib.Path, tier: str | None = "S",
             subset: str | None = None, out_dir: str | pathlib.Path | None = None, acceleration: bool = False) -> dict:
    pred_dir = pathlib.Path(pred_dir)
    meta = json.loads((pred_dir / "meta.json").read_text()) if (pred_dir / "meta.json").exists() else {}
    ...
    with h5py.File(pred_dir / "predictions.h5", "r") as fp:
        for r in idx.sort_values(["sample_id", "config"]).itertuples():
            ...
            m.update({"sample_id": r.sample_id, "config": r.config, "kind": r.kind, "task": KIND_TASK[r.kind], "family": r.family,
                      "split": r.split, "solve_time_s": r.solve_time_s, "speedup": efficiency.speedup(r.solve_time_s, m["inference_time_s"])})
            for c in ("test_ood_region", "test_ood_scale", "test_ood_table", "test_ood_contrast", "test_ood_synth2real"):
                if c in idx:
                    m[c] = bool(getattr(r, c))
            rows.append(m)
    df = pd.DataFrame(rows)
    result = {"model": meta.get("model", pred_dir.name), "meta": meta, "root": str(root), "tier": tier, "splits": splits,
              "subset": subset, "n_rows": int(len(df)), "evaluated_at": dt.datetime.now(dt.UTC).isoformat(timespec="seconds"),
              "per_task": {}, "per_sample": df.to_dict("records")}
    if len(df):
        num = [c for c in df.columns if df[c].dtype.kind == "f"]
        for task, g in df.groupby("task"):
            agg = {c: {"mean": float(np.nanmean(g[c])), "median": float(np.nanmedian(g[c])), "n": int(g[c].notna().sum())}
                   for c in num if g[c].notna().any()}
            agg["speedup"] = efficiency.summarize_speedup(g.solve_time_s.tolist(), g.inference_time_s.tolist())
    ...
    if acceleration:
        ...
        recs = []
        for s in srcs:
            recs += run_warm_start_eval(s, pred_dir / "predictions.h5", pred_dir / f"warm_start_{pathlib.Path(s).stem}.json")
...
PRIMARY = ["mae_log10eps", "rel_l2", "top5_iou", "pinch_recall", "spearman", "corridor_dice_q10", "reff_rel_error", "reff_spearman",
           "phys_kirchhoff_residual", "phys_focal_current_err", "phys_neg_fraction", "voltage_mae", "normalized_mae_log10eps"]
SECONDARY = ["ssim", "psnr_db", "mse", "top1_iou", "top10_iou", "reff_mae_log10", "reff_nn_agreement", "reff_symmetry", "phys_throughput_err"]


def markdown_table(result: dict) -> str:
    lines = [f"# Evaluation: {result['model']}", "",
    ...
    for task, agg in result["per_task"].items():
        ...
        for k in PRIMARY + SECONDARY:
            if k in agg:
                v = agg[k]
                lines.append(f"| {k}{' (secondary)' if k in SECONDARY else ''} | {v['mean']:.4g} | {v['median']:.4g} | {v['n']} |")
        sp = agg.get("speedup", {})
        if sp.get("speedup_median") is not None and not np.isnan(sp.get("speedup_median", np.nan)):
            lines.append(f"| speed-up vs solver (median / geomean) | {sp['speedup_median']:.3g} | {sp['speedup_geomean']:.3g} | |")
        lines.append("")
    if "acceleration" in result:
        s = result["acceleration"]["summary"]
        lines += ["## Solver acceleration (AMG-PCG warm start from predicted voltage)", "",
