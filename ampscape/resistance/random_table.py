...
def perturb_table(
    base: ResistanceTable, seed: int, log_sd: float = 0.5, table_id: str | None = None
) -> ResistanceTable:
    ...
    lc = {
        c: float(np.clip(v * np.exp(rng.normal(0.0, log_sd)), 1.0, base.r_max))
        for c, v in base.landcover.items()
    }
    d = base.model_dump()
    d["landcover"] = lc
    if base.roads is not None:
        d["roads"]["by_class"] = {
            c: float(np.clip(v * np.exp(rng.normal(0.0, log_sd)), 0.0, base.r_max))
            for c, v in base.roads.by_class.items()
        }
    if base.slope is not None:
        d["slope"]["per_degree"] = float(
            np.clip(base.slope.per_degree * np.exp(rng.normal(0.0, log_sd)), 0.0, 1.0)
        )
    d["table_id"] = table_id or f"random_{base.table_id}_{int(seed)}"
    d["description"] = (
        f"log-normal perturbation (sd={log_sd}) of {base.table_id} v{base.version}, seed {int(seed)}"
    )
    d["random"] = {
